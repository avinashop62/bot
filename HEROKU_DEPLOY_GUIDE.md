...
Since this is a bot (not a web app), scale the worker process and disable web:
...
- **409 Conflict (multiple bot instances)**: Stop any local bot instances. Ensure only one instance runs. If logs show "app[web.1]", run `heroku ps:scale web=0` and `heroku ps:scale worker=1`.
- **403 Forbidden on API**: The lottery API may block Heroku IPs. Try adding more headers or use a proxy if needed.
